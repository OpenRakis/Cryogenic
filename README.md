...
| `cs2` | `0xD000` | VGA driver (remapped by `DriverLoadToolbox`) |
| `cs3` | `0xE000` | PCM audio driver (remapped) |
| `cs4` | `0xE000` | MIDI music driver (remapped) |
| `cs5` | `0x0800` | Interrupt handlers |

`DriverLoadToolbox` temporarily overrides the game's memory allocator so that VGA, PCM, and MIDI drivers load at the 0xD000/0xE000/0xF000 boundaries listed above. After loading, it restores the allocator to its original state.

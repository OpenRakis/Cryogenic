...
Replacement files should be named to match the `name` value in `fingerprints.json` (filename without extension, case-insensitive). For example, a `name` of `Morning` will match `Morning.mp3`. Supported formats: MP3, OGG, WAV, FLAC, AAC.

`fingerprints.json` maps each song's 2-byte signature (4 hex characters) to a human-readable `name`. A default copy is provided in `assets_override/music/fingerprints.json` and is loaded from the `--MusicFolder` directory at runtime.

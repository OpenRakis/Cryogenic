...
using Cryogenic.Mt32Player.Services;
...
public sealed partial class MainWindowViewModel : ViewModelBase, IDisposable {
	...
	private Mt32MidiSynth? _synth;
	private DuneM32PlayerEngine? _player;
	...
	[ObservableProperty]
	private string _romPath = @"C:\Jeux\MT32\";

	[ObservableProperty]
	private string _m32Path = @"C:\Users\noalm\source\repos\Cryogenic\doc\DUNECDVF\C\DUNECD\DUNE.DAT_\MORNING.M32";

	[ObservableProperty]
	private string _status = "Select MT-32 ROM path and an M32 file.";
	...
	[ObservableProperty]
	private string _serverEndpoint = "http://127.0.0.1:8765/";
	...
	private async Task BrowseRomsAsync() {
		...
		IReadOnlyList<IStorageFolder> picked = await _window.StorageProvider.OpenFolderPickerAsync(new FolderPickerOpenOptions {
			Title = "Select MT-32 ROM folder",
			AllowMultiple = false
		});

		if (picked.Count > 0) {
			RomPath = picked[0].Path.LocalPath;
			Status = "ROM folder selected.";
		}
	}

	[RelayCommand]
	private async Task BrowseM32Async() {
		if (_window.StorageProvider is null) {
			return;
		}

		IReadOnlyList<IStorageFile> picked = await _window.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions {
			Title = "Select Dune M32 file",
			AllowMultiple = false,
			FileTypeFilter = [new FilePickerFileType("Dune M32") { Patterns = ["*.M32"] }]
		});

		if (picked.Count > 0) {
			M32Path = picked[0].Path.LocalPath;
			Status = "M32 file selected.";
		}
	}

	[RelayCommand]
	private void Load() {
		if (string.IsNullOrWhiteSpace(RomPath) || string.IsNullOrWhiteSpace(M32Path)) {
			Status = "Select both ROM path and M32 file first.";
			return;
		}
		...
		try {
			...
			_player.Load(M32Path);
			Status = "Loaded.";
			AddLog($"Loaded file '{M32Path}'.");
		} catch (Exception ex) {
	...
	private void Play() {
		try {
			if (string.IsNullOrWhiteSpace(M32Path)) {
				Status = "Select an M32 file first.";
				return;
			}
	...
	private void StartControlServer() {
		try {
			...
			_controlServer = new PlayerControlServer(
				...
				triggerTestNote: TriggerTestNote,
				sendMidi3: SendMidi3Diagnostic,
				sendMidi2: SendMidi2Diagnostic,
				sendMidi1: SendMidi1Diagnostic,
	...
	private void EnsurePlayerInitialized() {
		...
		// Validate ROM path first
		string romValidation = Mt32MidiSynth.ValidateRomPath(RomPath);
		if (!RomPath.EndsWith(".zip", StringComparison.OrdinalIgnoreCase)) {
			if (!Directory.Exists(RomPath)) {
				Status = $"ROM path not found: {RomPath}. {romValidation}";
				AddLog(Status);
				return;
			}
			string[] romFiles = Directory.GetFiles(RomPath, "*.ROM", SearchOption.TopDirectoryOnly);
			if (romFiles.Length == 0) {
				romFiles = Directory.GetFiles(RomPath, "*.rom", SearchOption.TopDirectoryOnly);
			}
			if (romFiles.Length == 0) {
				Status = $"No ROM files (.ROM) found in {RomPath}. {romValidation}";
				AddLog(Status);
				return;
			}
		}
		...
		try {
			_synth = new Mt32MidiSynth(RomPath);
			...
			_player = new DuneM32PlayerEngine(_synth);
			...
			AddLog($"MT-32 Synth initialized successfully from {RomPath}.");
		} catch (Exception ex) {
	...
	private void TriggerTestNote(byte channel, byte note, byte velocity, int durationMs) {
		EnsurePlayerInitialized();
		_player!.TriggerTestNote(channel, note, velocity, durationMs);
		AddLog($"Test note sent: ch={channel}, note={note}, vel={velocity}, durationMs={durationMs}.");
	}

	private void SendMidi3Diagnostic(byte status, byte data1, byte data2) {
		EnsurePlayerInitialized();
		_player!.SendMidi3Diagnostic(status, data1, data2);
		AddLog($"Injected MIDI3: {status:X2} {data1:X2} {data2:X2}.");
	}

	private void SendMidi2Diagnostic(byte status, byte data1) {
		EnsurePlayerInitialized();
		_player!.SendMidi2Diagnostic(status, data1);
		AddLog($"Injected MIDI2: {status:X2} {data1:X2}.");
	}

	private void SendMidi1Diagnostic(byte status) {
		EnsurePlayerInitialized();
		_player!.SendMidi1Diagnostic(status);
		AddLog($"Injected MIDI1: {status:X2}.");
	}
	...
	private object StopGoldenCapture() {
		...
		if (result is GoldenCaptureSummary summary) {
			AddLog($"Golden capture stopped (events={summary.EventCount}, dropped={summary.DroppedEvents}).");
		}
		return result;
	}

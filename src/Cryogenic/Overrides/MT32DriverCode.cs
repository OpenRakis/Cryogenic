...
public partial class Overrides {
	...
	public Action MidiOpen_F000_0103_F0103(int gotoAddress) {
		...
		if (_musicFolderPlayer is not null) {
			...
			_musicFolderPlayer.TryPlay(fingerprint);
			// ASM code that does volume control is very entangled with midi hardware.
			// Difficult in current state to decorrelate the two so we dont handle volume for overridden songs
			_musicFolderPlayer.Volume = 0.8f;
	...
	private void MidiHandleControlChangeEventWithDelta(byte statusByte, byte dataByte) {
		...
		if (dataByte == 0x07) {
			...
			byte currentVolume = CurrentVolume;
	...
	private byte CurrentVolume {
		get => Memory.UInt8[DS, 0x013D];
		set {
			Memory.UInt8[DS, 0x013D] = value;
			if (_musicFolderPlayer is not null) {
				_musicFolderPlayer.Volume = Math.Clamp(value / 255.0f, 0.0f, 1.0f);
			}
		}
	}
}